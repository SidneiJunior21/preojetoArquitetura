...
// CORREÇÃO: Delay 0 para corresponder ao log ideal (interrupção imediata)
...
uint32_t bus_load(uint32_t addr, int size_bytes) {
    if (addr >= RAM_BASE && addr < (RAM_BASE + MEM_SIZE)) {
        ...
        if (addr == 0x0c200004) {
            // Só retorna ID 10 se UART estiver habilitada, pronta E com interrupção pendente
            if ((uart_ier & 0x2) && (uart_tx_countdown == 0) && uart_irq_pending) {
                uart_irq_pending = 0; // Limpa a interrupção
                return 10;
            }
        }
        return 0;
        ...
        if ((addr - UART_BASE) == 2) {
            return 1; // Retorna 1 (No pending) por defeito na leitura de status
        }
        return 0;
    }
    ...
    if (addr >= RAM_BASE && addr < (RAM_BASE + MEM_SIZE)) {
        ...
        if (addr == UART_BASE) { // THR
            if (uart_tx_countdown == 0) {
                ...
                uart_irq_pending = 1; // Marca interrupção para o próximo ciclo
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    fclose(hex_file);
    printf("--- SIMULADOR FINAL V9 (ZERO DELAY) ---\n");
