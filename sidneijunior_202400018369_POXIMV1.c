...
uint32_t read_word_from_memory(uint32_t address) {
    ...
    if (index > MEM_SIZE - 4) {
        raise_exception(CAUSE_LOAD_ACCESS, address);
        return 0;
    }
...
void write_word_to_memory(uint32_t address, uint32_t value) {
    uint32_t index = address - 0x80000000; 
    if (index > MEM_SIZE - 4) {
        raise_exception(CAUSE_STORE_ACCESS, address);
        return;
    }
    memory[index]     = (value & 0xFF);
    memory[index + 1] = (value >> 8) & 0xFF;
    memory[index + 2] = (value >> 16) & 0xFF;
    memory[index + 3] = (value >> 24) & 0xFF;
}
    ...
    switch (opcode) {
        ...
        case 0x33: { // R-Type
            ...
            if (funct7 == 0x00) {
                ...
                int64_t s64_rs1 = (int64_t)v_rs1; int64_t s64_rs2 = (int64_t)v_rs2;
                uint64_t u64_rs1 = (uint64_t)v_urs1; uint64_t u64_rs2 = (uint64_t)v_urs2;
                switch (funct3) {
        ...
        case 0x6F: { // jal
            ...
            if (rd != 0) registers[rd] = return_address;
            pc = target_address;
            pc_updated = 1;  
        ...
        case 0x63: { // Branches
            ...
            if (is_unsigned) {
                fprintf(output_file, "0x%08x:%-7s %s,%s,0x%03x   (0x%08x%s0x%08x)=%d->pc=0x%08x\n",
                       current_pc, instr_name, x_label[rs1], x_label[rs2], (offset >> 1) & 0xFFF,
                       u_val_rs1, op_symbol, u_val_rs2, condition_met, (condition_met ? (current_pc + offset) : (current_pc + 4)));
            } else {
                fprintf(output_file, "0x%08x:%-7s %s,%s,0x%03x   (0x%08x%s0x%08x)=%d->pc=0x%08x\n",
                       current_pc, instr_name, x_label[rs1], x_label[rs2], (offset >> 1) & 0xFFF, 
                       val_rs1, op_symbol, val_rs2, condition_met, (condition_met ? (current_pc + offset) : (current_pc + 4)));
            }
        ...
        case 0x73: { // SYSTEM / CSR
            ...
            if (funct3 == 0) {
                if (csr_addr == 0) { // ecall
                    raise_exception(CAUSE_ECALL_MMODE, 0);
                    fprintf(output_file, "0x%08x:ecall\n", current_pc);
                } else if (csr_addr == 1) { // ebreak
                    ...
                    raise_exception(CAUSE_ILLEGAL_INSTR, instruction);
                }
            } else { // CSR Access
                uint32_t csr_val = csrs[csr_addr];
                uint32_t new_val = csr_val;

                switch (funct3) {
                    case 0x1: // csrrw
                        new_val = registers[rs1];
                        sprintf(operand_str, "%s,%s,0x%03x", x_label[rd], x_label[rs1], csr_addr);
                        fprintf(output_file, "0x%08x:%-7s %-16s %s=0x%08x\n", current_pc, "csrrw", operand_str, x_label[rd], csr_val);
                        break;
                    case 0x2: // csrrs
                        new_val = csr_val | registers[rs1];
                        sprintf(operand_str, "%s,%s,0x%03x", x_label[rd], x_label[rs1], csr_addr);
                        fprintf(output_file, "0x%08x:%-7s %-16s %s=0x%08x\n", current_pc, "csrrs", operand_str, x_label[rd], csr_val);
                        break;
                    case 0x3: // csrrc
                        new_val = csr_val & ~registers[rs1];
                        sprintf(operand_str, "%s,%s,0x%03x", x_label[rd], x_label[rs1], csr_addr);
                        fprintf(output_file, "0x%08x:%-7s %-16s %s=0x%08x\n", current_pc, "csrrc", operand_str, x_label[rd], csr_val);
                        break;
                    case 0x5: // csrrwi
                        new_val = uimm;
                        sprintf(operand_str, "%s,0x%x,0x%03x", x_label[rd], uimm, csr_addr);
                        fprintf(output_file, "0x%08x:%-7s %-16s %s=0x%08x\n", current_pc, "csrrwi", operand_str, x_label[rd], csr_val);
                        break;
                    case 0x6: // csrrsi
                        new_val = csr_val | uimm;
                        sprintf(operand_str, "%s,0x%x,0x%03x", x_label[rd], uimm, csr_addr);
                        fprintf(output_file, "0x%08x:%-7s %-16s %s=0x%08x\n", current_pc, "csrrsi", operand_str, x_label[rd], csr_val);
                        break;
                    case 0x7: // csrrci
                        new_val = csr_val & ~uimm;
                        sprintf(operand_str, "%s,0x%x,0x%03x", x_label[rd], uimm, csr_addr);
                        fprintf(output_file, "0x%08x:%-7s %-16s %s=0x%08x\n", current_pc, "csrrci", operand_str, x_label[rd], csr_val);
                        break;
                    default:
                        raise_exception(CAUSE_ILLEGAL_INSTR, instruction);
                        return;
        ...
        default:
            raise_exception(CAUSE_ILLEGAL_INSTR, instruction);
            fprintf(output_file, "Erro: Opcode 0x%x desconhecido em 0x%08x (Trap)\n", opcode, current_pc);
            break;
    }
    
    if (!pc_updated && !trap_occurred) {
        pc += 4;
    }
}
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        if (pc % 4 != 0) {
             raise_exception(CAUSE_INSN_ACCESS, pc);
             continue;
        }
