...
void execute_instruction(uint32_t instruction, uint32_t current_pc, FILE *output_file) {
    ...
    switch (opcode) {
        ...
        case 0x6F: { // jal
            ...
            fprintf(output_file, "0x%08x:%-7s %s,0x%05x        pc=0x%08x,%s=0x%08x\n", 
                   current_pc, "jal",
                   x_label[rd],
                   (offset >> 1) & 0xFFFFF, 
                   target_address,
        ...
        case 0x37: { // lui
            ...
            fprintf(output_file, "0x%08x:%-7s %s,0x%05x          %s=0x%08x\n",
                   current_pc, "lui",
                   x_label[rd],
                   (imm_u >> 12), 
                   x_label[rd],
        ...
        case 0x17: { // auipc
            ...
            if (rd != 0) registers[rd] = res;
            fprintf(output_file, "0x%08x:%-7s %s,0x%05x          %s=0x%08x+0x%08x=0x%08x\n",
                   current_pc, "auipc",
                   x_label[rd],
                   (imm_u >> 12) & 0xFFFFF,
                   x_label[rd],
        ...
        case 0x23: { 
            ...
            switch (funct3) {
                case 0x0: { // sb
                    ...
                    fprintf(output_file, "0x%08x:%-7s %s,0x%03x(%s)      mem[0x%08x]=0x%02x\n",
                           current_pc,
                           instr_name,
                ...
                case 0x1: { // sh
                    ...
                    fprintf(output_file, "0x%08x:%-7s %s,0x%03x(%s)      mem[0x%08x]=0x%04x\n",
                           current_pc,
                           instr_name,
                           x_label[rs2], (imm & 0xFFF), x_label[rs1],
                           address,
